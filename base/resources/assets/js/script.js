class Xamtirg {
    constructor() {
        ...
        Xamtirg.handleCounterUp();
        Xamtirg.initMediaIntegrate();
        if (XamtirgVariables && XamtirgVariables.authorized === '0') {
    ...
    static showNotice(messageType, message, messageHeader = '') {
        ...
        if (!messageHeader) {
            switch (messageType) {
                case 'error':
                    messageHeader = XamtirgVariables.languages.notices_msg.error;
                    break;
                case 'success':
                    messageHeader = XamtirgVariables.languages.notices_msg.success;
                    break;
            }
        }
    ...
    static handleError(data) {
        if (typeof (data.errors) !== 'undefined' && !_.isArray(data.errors)) {
            Xamtirg.handleValidationError(data.errors);
        } else {
            if (typeof (data.responseJSON) !== 'undefined') {
                if (typeof (data.responseJSON.errors) !== 'undefined') {
                    if (data.status === 422) {
                        Xamtirg.handleValidationError(data.responseJSON.errors);
                    }
                } else if (typeof (data.responseJSON.message) !== 'undefined') {
                    Xamtirg.showError(data.responseJSON.message);
                } else {
                    $.each(data.responseJSON, (index, el) => {
                        $.each(el, (key, item) => {
                            Xamtirg.showError(item);
                        });
                    });
                }
            } else {
                Xamtirg.showError(data.statusText);
            }
        }
    }
    ...
    countCharacter() {
        $.fn.charCounter = function (max, settings) {
            ...
            settings = $.extend({
                ...
                format: '(%1 ' + XamtirgVariables.languages.system.character_remain + ')',
                pulse: true,
                delay: 0
            }, settings);
            let p, timeout;
    ...
    static initResources() {
        ...
        Xamtirg.initDatePicker('.datepicker');
        if (jQuery().mCustomScrollbar) {
            Xamtirg.callScroll($('.list-item-checkbox'));
        }
    ...
    static initMediaIntegrate() {
        ...
        if (jQuery().rvMedia) {
            ...
            Xamtirg.gallerySelectImageTemplate = `<div class="list-photo-hover-overlay">
            ...
            const gallerySelectImages = function (files, $currentBoxList, excludeIndexes = []) {
                let template = Xamtirg.gallerySelectImageTemplate;
    ...
    handlePortletTools() {
        ...
        $('body').on('click', '.portlet > .portlet-title .fullscreen', event => {
            ...
            if (portlet.hasClass('portlet-fullscreen')) {
                ...
                let height = Xamtirg.getViewPort().height -
    ...
    static initCodeEditor(id, type = 'css') {
        ...
        $('.handle-tool-drag').mousedown(event => {
            ...
            $('body').attr('data-dragtool', _self.attr('id')).on('mousemove', Xamtirg.onDragTool);
            $(window).on('mouseup', Xamtirg.onReleaseTool);
        });
    }
    ...
    static onReleaseTool() {
        $('body').off('mousemove', Xamtirg.onDragTool);
        $(window).off('mouseup', Xamtirg.onReleaseTool);
    }
